  ...
  try {
    ...
    for (const question of questions) {
      if (!question.title || !question.description) {
        return res
          .status(400)
          .json({ message: "Title and description are required for each question" });
      }
      ...
      if (question.testCases) {
        for (const testCase of question.testCases) {
          if (!testCase.input || !testCase.output) {
            return res
              .status(400)
              .json({ message: "Each test case must have both input and output" });
          }
        }
      }
    }
    ...
    res
      .status(201)
      .json({ message: "Questions added successfully", questions: savedQuestions });
  } catch (error) {
