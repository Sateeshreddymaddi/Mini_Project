...
export default function StudentPage() {
  ...
  const [codingSubjects, setCodingSubjects] = useState([]); 
  const [showSubjectsModal, setShowSubjectsModal] = useState(false);
  const [showCodingSubjectsModal, setShowCodingSubjectsModal] = useState(false); 
  const [isExamDropdownOpen, setIsExamDropdownOpen] = useState(false);
  const [loadingSubjects, setLoadingSubjects] = useState(false);
  const [loadingCodingSubjects, setLoadingCodingSubjects] = useState(false); 
  ...
  useEffect(() => {
    const fetchCodingSubjects = async () => {
      setLoadingCodingSubjects(true);
      setCodingError(null);
      try {
        const res = await axios.get("http://localhost:5001/api/exam/coding-subjects");
        setCodingSubjects(res.data);
      } catch (error) {
        console.error("Error fetching coding subjects:", error);
        setCodingError("Failed to load available coding subjects. Please try again later.");
      } finally {
        setLoadingCodingSubjects(false);
      }
    };
    fetchCodingSubjects();
  }, []);
  ...
  const handleCodingExam = () => {
    ...
    setShowCodingSubjectsModal(true); 
  };
  ...
  return (
    <div className="h-screen w-screen bg-gradient-to-br from-gray-900 to-blue-900 overflow-hidden">
      <div className="flex h-full">
        ...
        {/* NEW: Coding Subjects Modal */}
        {showCodingSubjectsModal && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ duration: 0.3 }}
          ...
          >
            ...
            >
              <h2 className="text-2xl font-semibold mb-4 text-green-400">
                Available Coding Exams
              </h2>
              {loadingCodingSubjects ? (
                <p className="text-gray-400">Loading coding subjects...</p>
              ) : codingError ? (
                <p className="text-red-400">{codingError}</p>
              ) : (
                <ul className="space-y-3">
                  {codingSubjects.length > 0 ? (
                    codingSubjects.map((subject, index) => (
                      ...
                      >
                        <button
                          className="w-full text-left text-white"
                          onClick={() => {
                            setShowCodingSubjectsModal(false);
                            navigate(`/student/${studentId}/write-exam?subject=${subject}`);
                          }}
                        >
                          {subject}
                        </button>
                    ...
                    <li className="text-gray-400">No available coding exams</li>
                  )}
                </ul>
              )}
              <motion.button
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                onClick={() => setShowCodingSubjectsModal(false)}
                className="mt-4 w-full bg-red-600 text-white py-2 rounded-lg hover:bg-red-700 transition"
              >
                Close
              </motion.button>
